...
export default function MessageReportButton({
  conversationId,
  cookToken,
}: {
  ...
  async function submit() {
    ...
    await fetch(`/api/messages/${conversationId}/report`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ cook_token: cookToken, reason, details }),
    })
    setStatus('done')
  }
  ...
  if (status === 'done') {
    return (
      <p className="text-sm text-gray-500 text-center mt-4">
        Thank you — we will review this conversation shortly.
      </p>
    )
  }
